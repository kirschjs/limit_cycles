...
C
      ...
      MEFI=23
      WRITE(NOUT,1111)
1111  FORMAT(' KOBER_STREU_CIB VOM 09.11.2010')
      DO 329 I=1,NDIM1
329   SNSP(I)= 0.
...
C LS und TENSOR
      IF(MKC.EQ.6) ITEN=2
      IF(MKC.EQ.7 .OR. MKC.EQ.9) ITEN=4
...
C       NH(2+...I,K)= NUMMMER DES TEILCHEN VOM TYP K
 ...
 324  NHILF(K)=I
      MERKS(1,NFL,NFR)=0
      GOTO(326,326,326,326,328,328,328,328),MKC
...
C     SORTEN NICHT AENDERN
 ...
 330   CONTINUE
 328  GO TO (301,301,301,301,305,306,307,308),MKC
  301 LKC=1
      LLKC = 1
      GO TO 20
  305 LKC=4
...
308   LKC=2
   ...
   20 DO 1 MLKC=1,LLKC
      ...
      GOTO 12
19    IF (MKC.LT.6.OR.MKC.EQ.8) GOTO 10
...
11    DO 21 K=1,MAXLI
   ...
   12 CONTINUE
      NKC= MLKC
      KK=MLKC
      IF(MKC.EQ.6.OR.MKC.EQ.7 .OR. MKC.EQ.8 .OR. MKC.EQ.9)
     *         KK = (MLKC-1)/LKC + 1
C     KK NUMMERIERT DIE UNTEROPERATOREN DURCH
      IF(MLKC.GT.LKC) NKC = MLKC - LKC
      GOTO(200,200,200,200,200,201,202,200),MKC
C      CHECK OB MATRIXELEMENT FUER SPINDIFFERENZ MOEGLICH IST
  ...
  220 ISPZ=ISPZ+1
   ...
   29 NALG(K,3)=NALG(K,2)
      IFT=1
      GOTO(100,100,100,100,32,31,31,131),MKC
131   IFT=2
...
C    NKOR(.,5) (INVERSE) PERMUTATION ZU NKOR(.,2)*P0**-1
      LT1=NKOR(LT,4)
      KT1=NKOR(KT,4)
C      CHECK OB WW-TEILCHEN PROTONEN
      IF(MKC.NE.2.AND.MKC.NE.3) GOTO 117
      IF(NALG(KT,2).GT.2) GOTO 26
      IF(NALG(LT,2).GT.2) GOTO 26
C      CHECK OB WW-TEILCHEN NEUTRONEN
  117 IF(MKC.NE.4) GOTO 118
      IF(NALG(KT,2).LT.3) GOTO 26
      IF(NALG(LT,2).LT.3) GOTO 26
  118 DO 120 NF1=1,NH(1,1)
      DO 1121 KH=1,NH(2,1)
...
162   CONTINUE
   90 CONTINUE
      IF (MKC.LT.6.OR.MKC.EQ.10) GOTO 14
      IF(MKC.NE.9) GOTO 15
