...
C
      COMMON /COMY/ D(100)
C
      COMMON /CSPIN/ NALG(NZTMAX,3),ipair(2,NZTMAX**3),inbrp
C
...
C
      open(unit=123,FILE='INPAIR',STATUS='OLD')
      read(123,*) inbrp
      read(123,*) ((ipair(KI,KK),KI=1,2),KK=1,inbrp)
C
...
C
      DO 112 I=1,inbrp
      IF ((NALG(KT,3).EQ.ipair(1,I).AND.NALG(LT,3).EQ.ipair(2,I))
     *.or.(NALG(LT,3).EQ.ipair(1,I).AND.NALG(KT,3).EQ.ipair(2,I)))
     * THEN 
         VF = 1.
         goto 111
      ENDIF     
...
C      DC NACHEINANDER ERFOLGEN
      write(nout,*)'MKC: ',MKC,'   VF=',VF
      SNSP(J) = SNSP(J) + VF*NP(5)*IFT*VV(MKC,MOD(MLKC-1,LKC)+1)
