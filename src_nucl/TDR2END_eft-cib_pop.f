...
C    11. OPERATOR: 3N OLD URBANA9 ZENTRAL OP WITHOUT (ISO)SPIN
...
C     FAKU(I)=DBLE(I!!)
C      WRITE (NOUT,1111)
 1111 FORMAT(' STREU T-ENDMAT_CIB VOM 29.01.11')	
...
C NBAND1 != 10 for NLO EFT NN calc ---------------------------------
...
C      WRITE (NOUT,1113) (POTFAK(K) ,K=3,NZOP)
1113  FORMAT(  7E12.4)
111   MREG(1)=1
      NBAND1=ABS(NBAND1)
      NZBASV2=0
...
C
     ...
     1 (MMS(J,N),J=1,3),(MLWERT(J,N),J=1,5),(PAQ(J,N),J=1,MM),KPB(N)
      ...
      NZPAQ(N)=MM
      IF (IGAK.EQ.0) GOTO 1
C      WRITE(NOUT,196) N,(MMASSE(J,N),J=1,2),(MMLAD(J,N),J=1,2),
C     1     (MMS(J,N),J=1,3),(MLWERT(J,N),J=1,5),KPB(N)
...
C
     ...
     *               FORM='UNFORMATTED')
C      IF(NREL.EQ.1) WRITE(NOUT,*)' KEINE RELATIVISTIK MOEGLICH'
...
76    NUMK(MK)=10000.*OPWERT(1,MK)
      ...
      KK = NZQ(L)+1
      KL = NZQ(L+1)
75    WRITE (NOUT,106) L,(NUMK(M),M=KK,KL)
