...
C             GELESEN H.M.H.
C
C     LIST OF CALCULATED OPERATORS:
C     1. NORM 
...
C
  ...
  110 POTFAK(K) =1.
C
      H2M=197.31613**2/(938.211+939.505)
      E2R0=.51098*2.81785
...
C     WIRD NUR EINMAL DURCHLAUFEN 
      ...
      KK = IZQ(K) + M
C     CHECK NORM GT 0
      IF (OPWERT(MKCU,KK).GT.1.D-14) GOTO 1400
...
76    NUMK(MK)=10000.*OPWERT(1,MK)
      ...
      KK = NZQ(L)+1
      KL = NZQ(L+1)
C 75    WRITE (NOUT,105) L,(NUMK(M),M=KK,KL)
75    WRITE (NOUT,105) L,(OPWERT(1,M),M=KK,KL)
...
78    NUMK(MK)=10000.*OPWERT(2,MK)
      ...
      KK = NZQ(L)+1
      KL = NZQ(L+1)
77    WRITE (NOUT,105) L,(OPWERT(2,M),M=KK,KL)
C77    WRITE (NOUT,105) L,(NUMK(M),M=KK,KL)
      WRITE(NOUT,305)
...
C     print "Eigenwerte des Hamiltonians"
      WRITE (NOUT,1101)
      WRITE (NOUT,1000) ( Q(M),M=1,NX )
...
C      NZZ = NX
      ...
      WRITE(NOUT,*)' EIGENVECTORMATRIX'
      CALL SCHEMA(ZH(1,1),NX,NX,NDIMD,5)

      OPEN(UNIT=29,FILE='MATOUTV',STATUS='REPLACE',
     *   FORM='UNFORMATTED')
      WRITE(29) 
     * ((ZH(N,M),N=1,NX),M=1,NX)
      CLOSE(UNIT=29,STATUS='KEEP')
