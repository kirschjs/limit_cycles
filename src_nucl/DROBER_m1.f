...
C
      COMMON /COMY/ D(100)
C
      COMMON /CSPIN/ NALG(NZTMAX,3),itripl(3,NZTMAX**3),inbrt
C
...
C
      open(unit=123,FILE='INTRIP',STATUS='OLD')
      read(123,*) inbrt
      read(123,*) ((itripl(KI,KK),KI=1,3),KK=1,inbrt)
C
...
C     V = 1/6(V_12+V_34) + 1/3(V_14+V_23)
32    VF = 0.                                            

      DO 112 I=1,inbrt
      IF ((NALG(IT,3).EQ.itripl(1,I).AND.NALG(JT,3)
     *.EQ.itripl(2,I).AND.NALG(KT,3).EQ.itripl(3,I))
     *.or.(NALG(IT,3).EQ.itripl(1,I).AND.NALG(JT,3)
     *.EQ.itripl(3,I).AND.NALG(KT,3).EQ.itripl(2,I))
     *.or.(NALG(IT,3).EQ.itripl(2,I).AND.NALG(JT,3)
     *.EQ.itripl(1,I).AND.NALG(KT,3).EQ.itripl(3,I))
     *.or.(NALG(IT,3).EQ.itripl(2,I).AND.NALG(JT,3)
     *.EQ.itripl(3,I).AND.NALG(KT,3).EQ.itripl(1,I))
     *.or.(NALG(IT,3).EQ.itripl(3,I).AND.NALG(JT,3)
     *.EQ.itripl(1,I).AND.NALG(KT,3).EQ.itripl(2,I))
     *.or.(NALG(IT,3).EQ.itripl(3,I).AND.NALG(JT,3)
     *.EQ.itripl(2,I).AND.NALG(KT,3).EQ.itripl(1,I)))
     * THEN 
         VF = 1.
         goto 111
      ENDIF     
...
111   continue
      if (VF.ne.0) then
      write(nout,*)'ijk:',IT,JT,KT, 
     *                  NALG(IT,3), NALG(JT,3),NALG(KT,3),VF
      endif
      goto 100
C
...
C      write(nout,*)'MKC: ',MKC,'   VF=',VF
      SNSP(J) = SNSP(J) + VF*(NP(5)*IFT)*VV(MKC,NKC)
