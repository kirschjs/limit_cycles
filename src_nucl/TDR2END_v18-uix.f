...
C      CHECK AUF GLEICHE RADIALDREHIMPULSE
  ...
  400 CONTINUE
   ...
   17 CONTINUE
      MM = LUM(MM+1,N)
      KP2=KPK(N)*2
      IF(MREG(3).EQ.0 .OR. ABS(POTFAK(1)).EQ.0.) THEN
      MLAD(1,N) = 0
      MLAD(2,N) = 0
      E2K(N) = .0
      ENDIF
...
C     WIRD NUR EINMAL DURCHLAUFEN 
      ...
      KK = IZQ(K) + M
C     CHECK NORM GT 0
C     ECCE: threshold lowerd from 10^-13 to 10^-15 
      IF (OPWERT(MKCU,KK).GT.1.D-15) GOTO 1400
...
C      CALL SCHEMA(DM(1,1,2,1),NX,NX,NDIMD,5)
C      CALL SCHEMA(DM(1,1,2,2) ,NX,NX,NDIMD,5)
      DO 322 M=1,NX
      DO 322 N=1,NX
      ENt(N,M) =  DM(N,M,2,1)
      EN(N,M)  =  DM(N,M,2,1)
