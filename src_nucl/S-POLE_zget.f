...
C  ENERGIEN K�NNEN WIE ZUVOR F�R NOPT = 0 BERECHNET WERDEN ODER AUCH   C
...
C 7         NAD1,..,NAD10                                              C
C 8       NZRW                                                         C
C 9       FRR:0.06,RHG:8.0,RHF:1.0,CDEF:0.x                            C 
C 10         1  2  3  4   (ISTR)                                       C   
...
C                                                                      C
C    - FUER NOPT= 0: BLEIBT DER INPUT UNVER�NDERT                      C
...
C                            => DELTA WIRD SOLANGE VERGROE�ERT,        C
...
10     WRITE(NOUT,3001) LWERT(I),REDM(I),E2K(I),JWERT(I),MASSE(1,I),
     1  MASSE(2,I),MLAD(1,I),MLAD(2,I),EBIN(I),WNORM(I)
     2 ,RHUM(1,I),RHUM(2,I)
3001  FORMAT(1X,I3,2G15.5,5I3,4G15.5)
...
12    READ(NBAND2) (PAR(M,I),M=1,NZPWM)
...
C     NAD6 = 0 KEIN STREICHEN VON EIGENVEKTOREN IN RRANDB
C     NAD6 = N MAXIMAL N EIGENVEKTOREN KOENNEN AUTOMATISCH GESTRICHEN WERDEN
...
c      IF (NAD6.NE.0) WRITE (NOUT,5711) NAD6
c 5711 FORMAT(/8X,18H ES WERDEN MAXIMAL,I2,25H EIGENVEKTOREN GESTRICHEN)
      IF(NOPT.NE. 0 .OR. NAD3.EQ.0) GOTO 71
   71 READ (INPUT,1000)  NZRW
      IF(NZRW.GT.NZRWMA) GOTO 1800
      READ (INPUT,1001) FRR,RHG,RHF,CDEF
...
C
C     Vorbereitung f�r asomptot. Beitr�ge:
C     es werden Hankelfunktionen verwendet 
C     es ergeben sich nun weitere Beitr�ge,
C     vorher:
...
C
     ...
     *          BSIN(NZKBMA),BPHAS(NZKBMA)
      DIMENSION DIFF(NDIM),NDIFF(NDIM),NBVSTR(NDIM),CRI(NZKBMA),
     *          EINS(NZKBMA,NZKBMA)
C
      CHARACTER*1 TRANS
      SAVE /DEK/
...
c
 ...
 339  CONTINUE
C
C     Sortieren und Streichen von Eigenvektoren, falls NAD6.NE.0
      MM=1
      IF(NAD6.EQ.0) GO TO 30
C     Sortieren VON EIGENVEKTOREN
      DO 102 K=1,NX
      AN=ABS(ENERGI-RQ(K))
      NDIFF(K)=K
      DIFF(K)=1./AN
  102 CONTINUE
      DO 153 K=2,NX
C     ORDNEN VON DIFF DER GROESSE NACH
      KK=K-1
      T=DIFF(KK)
      NT=NDIFF(KK)
      M=KK
      DO 105 L=K,NX
      IF (DIFF(L).LT.T) GOTO 105
      T=DIFF(L)
      NT=NDIFF(L)
      M=L
  105 CONTINUE
      IF (KK.EQ.M) GO TO 153
      DIFF(M)=DIFF(KK)
      NDIFF(M)=NDIFF(KK)
      DIFF(KK)=T
      NDIFF(KK)=NT
  ...
  46  FORMAT(/)
      DO 5753 K=1,NAD6
      NBVSTR(K)=NDIFF(K)
 5753 WRITE (NOUT,5752) K,NDIFF(K),DIFF(K)
 5752 FORMAT (I4,3H //,I4,3H - ,E18.9)
      GOTO 30
c     sortieren beendet
  130 CONTINUE
C     STREICHEN DER EIGENVEKTOREN
      NSTREI=NDIFF(MM)
      write(nout,*)'mm=',mm,nstrei
      AN1=1./(ENERGI-RQ(NSTREI))
      DO 112 L=1,NO
      DO 112 K=1,NO
      HSMM(K,L)=HSMM(K,L)-ST(NSTREI,K)*ST(NSTREI,L)*AN1
      HSPM(K,L)=HSPM(K,L)-SS(NSTREI,K)*ST(NSTREI,L)*AN1
      HSMP(K,L)=HSMP(K,L)-ST(NSTREI,K)*SS(NSTREI,L)*AN1
  112 HSPP(K,L)=HSPP(K,L)-SS(NSTREI,K)*SS(NSTREI,L)*AN1
c
      IF(NAD7.LT.4) GO TO 1339
      CALL WRIMATC(HSPP,NZKBMA,NO,HTEX(1))
      CALL WRIMATC(HSPM,NZKBMA,NO,HTEX(2))
      CALL WRIMATC(HSMP,NZKBMA,NO,HTEX(3))
      CALL WRIMATC(HSMM,NZKBMA,NO,HTEX(4))
...
c     IF(CRI(MM).GT.CRI(MM-1)) GOTO 3741
      ...
      WRITE (NOUT,5757) MM
5757  FORMAT (1X,' ANZAHL DURCHLAEUFE (MM): ',I3)
      IF(MM.LE.NAD6) GO TO 130
3741  WRITE (NOUT,2750) CRI(MM),CRI(MM-1)
2750  FORMAT (2X,' CRI(MM): ',E13.6,' CRI(MM-1): ',E13.6)
5741  CONTINUE
C     Test auf Streichen zu Ende
      WRITE (NOUT,5756) MM
5756  FORMAT (1X,' ANZAHL DURCHLAEUFE danach (MM): ',I3)
      MANZ=MM-1
      WRITE (NOUT,2756) MANZ
2756  FORMAT (1X,' ANZAHL DURCHLAEUFE danach (MANZ): ',I3)
C
...
C
 ...
 43   ZZ(K,L)=AMAT(K,L)
      ...
      DO 60   K = 1,NO
 60   ZZ(K,K) = 57.295779513 * ATAN(ZZ(K,K))
      WRITE(NOUT,26)
...
C     ZC IST DIE TRANSFORMIERTE K-MATRIX
      ...
      DO 80   K = 1,NO
80    ZC(K,K) = 57.295779513 * ATAN(ZC(K,K))
      WRITE(NOUT,26)
...
C
     ...
     *        IELMA
C
      COMMON /DEK/ A(NZKBMA,NZKBMA,2), D(NDIM,NZKBMA,2)
C
      COMMON /DEKDD/ DD(NDIM,NZKBMA,2)
C
      COMMON /NAD0/ NAD5, NAD6, NAD7, IBAC
C
      DIMENSION ZW(NDIM,NZKBMA)
C
      SAVE /DEK/, /DEKDD/
C
      DO 1 MTD=1,2
      ...
      DO 13 I=1,NZPWX
      AN=ENERGI-RQ(I)
      IF(ABS(AN).LT.CDEF)  THEN
        WRITE(NOUT,*) 'EIGENVECTOR',I,' SKIPPED'
c     dadurch wird der dd-koeffizient der vorherigen energie verwendet        
        GOTO 13
      ENDIF
      ZW(I,L)=0.
      DO 12 M=1,NZAOK
      ZW(I,L)=ZW(I,L)+A(L,M,MTD)*SS(I,M)
12    CONTINUE
C     
      DD(I,L,MTD)=(ST(I,L)-ZW(I,L))/AN
13    CONTINUE
11    CONTINUE
      CALL TRAFO(MTD)
      IF(NAD1.LE.0) GOTO 1
...
1     CONTINUE
      RETURN
      END
      SUBROUTINE TRAFO(MTD)
      IMPLICIT REAL*8(A-H,O-Z) 
      COMPLEX*16 A,D,ZW, DD
C
      INCLUDE 'par/verma'
C
      COMMON /APC/ FRR, BUM(NZKBMA), NZP(NZKMAX), WNORM(NZKMAX),
...
C
      ...
      ZW(I,L)=0.
      DO 30 J=1,NZPWX
      ZW(I,L)=ZW(I,L)+DD(J,L,MTD)*EV(I,J)
30    CONTINUE
31    CONTINUE
20    CONTINUE
      DO 40 L=1,NZAOK
...
C
     ...
     *       E2K(NZKMAX), BNORM, J_RMAT
C
      DIMENSION CWF(NZKBMA,NZKMAX,NZPARM), METH(2), SW(100,NZPARM)
C
      SAVE METH, EIHALF, CI
C
...
20    CONTINUE
       ...
       DO 22 M=1,NZPW
       I=I+1
22     CWF(KK,KL,M)=CWF(KK,KL,M)+EIHALF*D(I,KK,MTD)
10     CONTINUE
       DO 70 K=1,NO
       DO 69 L=1,NZKA
       NZPW=NZP(L)
      IF(IWEFU.GT.1) WRITE(NOUT,1000) K,L,(CWF(K,L,M),M=1,NZPW)
1000  FORMAT(1X,2I3,/,(1X,10E13.6))
