...
C     28. OPERATOR: TENSOR-TENSOR RANG=2   K OM
...
C
     ...
     *       IDRU, NB15, IPLO
C
      COMMON /XH/ E2K(NZKMAX),PAQ(MZPARM,NZBMAX),DN(MZPARM,MZPARM,2),
     *          OPW(NZOPER), UMKOF(NZKMAX,NZBMAX),
     *          KPB(NZBMAX),  LREG(NZOPER), NZPAQ(NZBMAX),
     *          MMASSE(2,NZBMAX), MMLAD(2,NZBMAX),
     *          MS(3,NZKMAX), NKAENT(NZKMAX),
     *          LUM(MZPARM+1,NZKMAX), 
...
C
      ...
      character*80 cdiquaout, dqiname
      DATA CQUAOUT /'TQUAOUT.'/, CDQUAOUT /'TDQUAOUT.'/
      DATA CDIQUAOUT /'TDIQUAOUT.'/
C
...
C     FAKU(I)=DBLE(I!!)
      WRITE (NOUT,1111)
 1111 FORMAT(' STREU T-ENDMAT_I VOM 08.02.06')	
      write(nout,*)'NDIMD=',NDIMD
C
      D(1)=.0
      D(2)=.0
      DO 199 I=2,99
...
C
     ...
     *    NZOP.NE.31 .AND. NZOP.NE.42)  STOP 2
      IF(NZOP.EQ.23) THEN
          DO 137 K=1,7
...
113   LREG(K)=0
     ...
     1  (NZRHO(K),K=1,NZF)
      IF(IDREI_U.EQ.1) READ(11) NZF,(LREG(K),K=15,31),NZBASV3_U,
     1  (NZRHO(K),K=1,NZF)
      IF(IDREI_I.EQ.1) READ(22) NZF,(LREG(K),K=32,NZOPER),NZBASV3_I,
     1  (NZRHO(K),K=1,NZF)
C 
      write(nout,*)'kopf gelesen!'
      MKCU=1
      MKCO=NZOP
...
112   MREG(K)=MREG(K)*LREG(K)
 ...
 1010 FORMAT(' OPERATOREN ',/,(10I3))
      ...
      IF (KK.GT.NZUMAX) STOP 'NZUMAX ZU KLEIN'
...
C       KANALDEFINITION
      J = 0
      DO 161 M=1,NZBASV
      IF (ABS(UMKOF(N,M)).LT.1.D-14) stop 161
      J = J + 1
      NUMK(J) = M
      UMK(J) = UMKOF(N,M)
 161  CONTINUE
...
C
 ...
 193  NCOF(L,N) = 1
   ...
   27 CONTINUE
      J = 0
      DO 25 M=1,NZBASV
      IF (ABS(UMKOF(N,M)).LT.1.D-18) GOTO 25
      J = J + 1
      NUMK(J) = M
      UMK(J) = UMKOF(N,M)
  25  CONTINUE
  ...
  410 CONTINUE
      LUM(MM+1,N)=J1
      GO TO 84
   87 IF(NZP(N).NE.NZPAQ(J)) stop 16
      DO 713   M = 1,MM
      IF(ABS(PAR(M,N)-PAQ(M,J)).GT..00001) stop 16
  713 CONTINUE
      IF(KEIND.LT.0) GOTO 84
      IF(MS(3,N).NE.MMS(3,J)) stop 16
      IF(LWERT(5,N).NE.MLWERT(5,J)) stop 16
      IF(IABS(MASSE(1,N)-MMASSE(1,J))+IABS(MASSE(2,N)-MMASSE(2,J))
     &.NE.0) stop 16
      IF(LWERT(4,N).NE.MLWERT(4,J)) stop 16
   84 CONTINUE
...
C
 ...
 9995    dqname = cdquaout(1:k-1) // fnumber(i:j)
         do k=1, 80
            if(cdiquaout(k:k).eq.' ') goto 9996
         end do
 9996    dqiname = cdiquaout(1:k-1) // fnumber(i:j)
       CLOSE(UNIT=21,STATUS='KEEP')
       CLOSE(UNIT=31,STATUS='KEEP')
       CLOSE(UNIT=41,STATUS='KEEP')
       if(izwei.eq.1)
     1open(unit=21,file=qname, status='unknown', form='unformatted')
       if(idrei_u.eq.1)
     1open(unit=31,file=dqname, status='unknown', form='unformatted')
       if(idrei_i.eq.1)
     1open(unit=41,file=dqiname, status='unknown', form='unformatted')
        if(izwei.eq.1) then
           nband21= 21
           rewind nband21
        endif
        ...
        if(idrei_i.eq.1) then
c      WRITE(NOUT,*) 'Oeffne file: ',dqiname
c      WRITE(NOUT,*) 'MKCU/MKCO = ',MKCU,'/',MKCO
           nband41= 41
           rewind nband41
        endif
      DO 139 MFR=1,MFL
...
C
      ...
      DO 41, MKC=1,NZOP
      KPUTZ=2
      IF(MKC.EQ.1 .OR. MKC.EQ.15 .OR. MKC.EQ.32) KPUTZ=1
C     LOOP OPERATOREN
      IF(MKC.GT.14) NBAND21=NBAND31
      IF(MKC.GT.31) NBAND21=NBAND41
C
...
99    FAK1 = UMKOF(KANL,NUML)*UMKOF(KANR,NUMR)
      FAK2 = UMKOF(KANL,NUMR)*UMKOF(KANR,NUML)
       IF (ABS(FAK1)+ABS(FAK2).LT.1.D-13) GOTO 142
...
45    IF(MEMPT.GT.1) GOTO 50
      ...
      IF(MKC.NE.1 .AND. MKC.NE.15 .AND. MKC.NE.32) GOTO 49
      OPWERT(NZOPER+1,KK) =
...
C     WIRD NUR EINMAL DURCHLAUFEN 
      ...
      KK = IZQ(K) + M
C     CHECK NORM GT 0
      IF (OPWERT(MKCU,KK).GT.1.D-16) GOTO 1400
...
C     KEIN AUSDRUCK DER ENERGIEN FUER KAUSD .LT. 0
      I=0
      LU=2
      IF(IZWEI.EQ.0 .AND. IDREI_U.EQ.1) LU=16
...
1016  FORMAT(1X,'VOR ORTHO')
      ...
      CALL SCHEMA(DM(1,1,1,2),MMM,MMM,NDIMD,MIN(MMM,10))
...
1050  FORMAT(//,1X,'ENDE DER RECHNUNG VON ENDMAT')
      END
C
C   16 WRITE(NOUT,1006)  N,J
C      STOP
C808    WRITE(NOUT, 809)NZBASV,NZBMAX
C809    FORMAT(' TOO MANY BASISVECTORS',2I10)
C       STOP
C901   WRITE(NOUT, 902)NZKA,NZKMAX
C902   FORMAT(' TOO MANY CHANNELS',2I5)
C      STOP
C905   WRITE(NOUT, 906)MM,NDIMD
C906   FORMAT(' TOO MANY RADIALPARAMETERS*CHANNELS',2I5)
...
C     BESTIMMUNG DER KANAELE,DIE ZUE EINEM POLYNOMKANAL GEHOEREN
      GAMMA(1)= SQRT(3.1415926535897932384D0)
      GAMMA(2)=1.
      DO 100 K=1,28
  100 GAMMA(K+2)= DBLE(K)*GAMMA(K)*.5D0
...
2     CONTINUE
      ...
      DO 61 M=1,MMK
      KK = NZQ(K) + M
      AA= SQRT(2.D0*PAR(M,K)/REDM(K))
   61 HILF(KK )=(2.D0*(AA**MN)/GAMMA(MN))*OPWERT(NZOPER+1,KK)
