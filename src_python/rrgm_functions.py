...
def plotphas(infi='PHAOUT', oufi='tmp.pdf', outtxt='tmp.txt', chs=[], titl=''):
    ...
    for cha in phases:
        ...
        #np.savetxt(outtxt, np.column_stack((en, eta)))
      # cross-section calculation an dplot 
        
        #output_pdf = f'{cha}_cross_section.pdf'
        file = channel_outtxt
        J=0
        s1=0
        s2=0
        output_pdf='cross_sections.pdf'

        with PdfPages(output_pdf) as pdf:
#        for file in files:
            try:
                # Load eta and energy data
                ...
                # output_file = 'cross_section_' + cha + '.txt'
                # np.savetxt(output_file, np.column_stack((en, cross_section)), header='Energy CrossSection')
                
                # plot_data_file = 'cross_section_' + cha + '_plot.dat'
                # np.savetxt(plot_data_file, np.column_stack((en, cross_section)), header='Energy CrossSection')
                output_file = 'cross_section_' + str(file.split('_')[0]) + '.txt'
                np.savetxt(output_file, np.column_stack((energy, cross_section)), header='Energy CrossSection')

                
                plot_data_file = 'cross_section_' + str(file.split('_')[0]) + '_plot.dat'
                np.savetxt(plot_data_file, np.column_stack((energy, cross_section)), header='Energy CrossSection')

                
                fig, ax = plt.subplots()
                ax.plot(energy, cross_section, linestyle='-', marker='o', label='Cross Section')
                ax.set_xlabel('Energy [MeV]')
                ax.set_ylabel('Cross Section')
                ax.set_title(f'Cross Section vs Energy for {file.split("_")[0]}')
...
def plotphas_new(infi='PHAOUT', oufi='tmp.pdf', outtxt='tmp.txt', chs=[], titl=''):
    phases = {}
    etas = {}
    ...
    for line in tmp:
        for ch in chs:
            if ((ch[0] == line[2]) & (ch[1] == line[3])):
                ...
                try:
                    Exx = float(line[0]) if int(
                        line[2]) == 1 else phases[chastrTH][-1][0]
                    tmp_phas = float(line[10])
                    tmp_eta = float(line[9])
                    phases[chastr].append([Exx, tmp_phas])
                    etas[chastr].append([Exx, tmp_eta])
    ...
    with PdfPages('cross_sections.pdf') as pdf:
        for cha in phases:
            ...
            # Cross-section calculation and plot
            J = 0
            s1 = 1/2
            s2 = 0
...
def plotphas_new2(infi='PHAOUT', oufi='tmp.pdf', outtxt='tmp.txt', chs=[], titl=''):
    ...
    with PdfPages(oufi) as pdf:
        for cha in phases:
            ...
            ax2.plot(en, eta, linestyle=stylel, linewidth=linew)
            ...
            try:
                cross_section = ((2 * J + 1) / ((2 * s1 + 1) * (2 * s2 + 1))) * (eta ** 2) / en
                cross_section_file = f'cross_section_{cha}.txt'
                np.savetxt(cross_section_file, np.column_stack((en, cross_section)), header='Energy CrossSection')
    ...
    # Plot cross sections for all files generated
    cross_section_files = glob.glob('cross_section_*.txt')
    with PdfPages('all_cross_sections.pdf') as pdf:
        for file in cross_section_files:
            en, cross_section = np.loadtxt(file, unpack=True)
            fig_cross_all, ax_cross_all = plt.subplots()
            ax_cross_all.plot(en, cross_section, linestyle='-', marker='o', label=file)
            ax_cross_all.set_xlabel('Energy [MeV]')
            ax_cross_all.set_ylabel('Cross Section')
            ax_cross_all.set_title('All Cross Sections')
            ax_cross_all.legend()
            ax_cross_all.grid(True)
            pdf.savefig(fig_cross_all)
            plt.close(fig_cross_all)
####################################################
def calculate_and_plot_cross_sections(input_pattern='*_tmp.txt', output_pattern='cross_section_', J=0, s1=0, s2=0, output_pdf='cross_sections.pdf'):
    file = input_pattern
...
#        for file in files:
        try:
            ...
            # Calculate cross section
            cross_section = ((2 * J + 1) / ((2 * s1 + 1) * (2 * s2 + 1))) * (eta ** 2) / energy

            
            output_file = output_pattern + file.split('_')[0] + '.txt'
            np.savetxt(output_file, np.column_stack((energy, cross_section)), header='Energy CrossSection')

            
            plot_data_file = output_pattern + file.split('_')[0] + '_plot.dat'
            np.savetxt(plot_data_file, np.column_stack((energy, cross_section)), header='Energy CrossSection')
